...
function normalizeUi(raw) {
  ...
  if (!raw.nav && !raw.footer && (typeof raw.disclaimer !== 'object' || Array.isArray(raw.disclaimer))) {
...
export async function resolveConfig(projectRoot) {
  ...
  // Static asset base URL for media (audio, pdf) hosted off-site.
  // Priority for slug: explicit config.staticSlug -> PROJECT_SLUG env -> hostname's first label.
  // Without STATIC_SUBDOMAIN, staticBaseUrl is empty (paths stay as-is for dev).
  const staticSlug =
    userConfig.staticSlug
    || process.env.PROJECT_SLUG
    || (config.siteDomain ? config.siteDomain.split('.')[0] : '');
  const staticSubdomain = process.env.STATIC_SUBDOMAIN || '';
  config.staticSlug = staticSlug;
  config.staticBaseUrl = staticSubdomain && staticSlug
    ? `https://${staticSubdomain.replace(/^https?:\/\//, '').replace(/\/+$/, '')}/${staticSlug}`
    : '';
