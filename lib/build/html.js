...
export function generateChapterHtml(config, chapter, lang, glossary, references, provenance, allChapters, chapterSlugMap, media) {
  ...
  // URL building
  const canonicalUrl = `${config.siteUrl}/${lang}/chapters/${slug}.html`;
  ...
  const head = generateHead(config, {
    ...
    hreflangUrlBuilder: (l) => {
      ...
      return s ? chapterUrlFromSlug(config, l, s).replace(/^\//, config.siteUrl + '/') : '';
    }
  });
...
export function generateIndexHtml(config, lang, chapters, media) {
  ...
  const langSwitcher = config.languages
    .map(l => {
      ...
      return `<a href="/${l}/"${isActive} onclick="localStorage.setItem('lang','${l}')" aria-label="${ariaLabel}">${l.toUpperCase()}</a>`;
    })
    .join(' | ');
  ...
  const head = generateHead(config, {
    ...
    canonicalUrl: `${config.siteUrl}/${lang}/`,
    lang,
    ogType: 'book',
    hreflangUrlBuilder: (l) => `${config.siteUrl}/${l}/`
  });
...
export function generateAboutHtml(config, lang, about, allChapters, chapterSlugMap) {
  ...
  const head = generateHead(config, {
    ...
    canonicalUrl: `${config.siteUrl}/${lang}/about.html`,
    lang,
    ogType: 'website',
    hreflangUrlBuilder: (l) => `${config.siteUrl}/${l}/about.html`
  });
...
${termsHtml}
  ...
  const head = generateHead(config, {
    ...
    canonicalUrl: `${config.siteUrl}/${lang}/glossary.html`,
    lang,
    ogType: 'website',
    hreflangUrlBuilder: (l) => `${config.siteUrl}/${l}/glossary.html`
  });
