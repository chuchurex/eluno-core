...
function generateMediaToolbar(config, chapterNum, media, ui) {
  if (!media) return '';
  const chapterMedia = media[String(chapterNum)];
  if (!chapterMedia) return '';

  const audioUrl = resolveMediaUrl(chapterMedia.audio, config);
  const pdfUrl = resolveMediaUrl(chapterMedia.pdf, config);
  const youtubeUrl = chapterMedia.youtube || '';
  ...
  if (hasPdf) {
    html += `                    <a href="${pdfUrl}" class="ch-media-icon" title="${ui.downloadPdf}" download>${MEDIA_SVG.pdf}<span class="ch-media-label">PDF</span></a>\n`;
  }

  if (hasYoutube) {
    html += `                    <a href="${youtubeUrl}" class="ch-media-icon" target="_blank" rel="noopener" title="YouTube">${MEDIA_SVG.youtube}<span class="ch-media-label">YouTube</span></a>\n`;
  }
...
function generateHomepageMediaToolbar(config, media, ui) {
  if (!media || !media.all) return '';
  const allMedia = media.all;

  const audioUrl = resolveMediaUrl(allMedia.audio, config);
  const pdfUrl = resolveMediaUrl(allMedia.pdf, config);
  const youtubeUrl = allMedia.youtube || '';
  ...
  if (hasAudio) {
    html += `          <a href="${audioUrl}" class="ch-media-icon" title="${ui.listenAudio}" download>${MEDIA_SVG.audio}<span class="ch-media-label">MP3</span></a>\n`;
  }
  if (hasPdf) {
    html += `          <a href="${pdfUrl}" class="ch-media-icon" title="${ui.downloadPdf}" download>${MEDIA_SVG.pdf}<span class="ch-media-label">PDF</span></a>\n`;
  }
...
export function generateChapterHtml(config, chapter, lang, glossary, references, provenance, allChapters, chapterSlugMap, media) {
  ...
  const mediaToolbar = config.features.mediaToolbar
    ? generateMediaToolbar(config, chapter.number, media, ui)
    : '';
...
export function generateIndexHtml(config, lang, chapters, media) {
  ...
  // Media toolbar
  const mediaToolbar = config.features.mediaToolbar
    ? generateHomepageMediaToolbar(config, media, ui)
    : '';
