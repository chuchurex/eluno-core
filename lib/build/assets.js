 ...
 *
 * When baseLangPrefix: true  → copies baseLang/index.html to root + lang detection
 * When baseLangPrefix: false → baseLang index is already at root, just injects lang detection
 */
export function generateRootIndex(config) {
  const rootIndexPath = join(config.outputDir, 'index.html');

  let rootHtml;
  if (config.baseLangPrefix) {
    // Base lang has prefix — copy its index to root
    const baseLangIndexPath = join(config.outputDir, config.baseLang, 'index.html');
    if (!existsSync(baseLangIndexPath)) return;
    rootHtml = readFileSync(baseLangIndexPath, 'utf8');
  } else {
    // Base lang is already at root
    if (!existsSync(rootIndexPath)) return;
    rootHtml = readFileSync(rootIndexPath, 'utf8');
  }
