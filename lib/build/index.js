...
export async function build(options = {}) {
  ...
  // Compile SCSS (sass must be installed as peer dependency)
  compileSass(config);
...
/**
 * Compile SCSS from core to dist/css/main.css.
 * Consumer can override by placing main.scss in src/scss/.
 */
function compileSass(config) {
  const consumerScss = join(config.projectRoot, 'src', 'scss', 'main.scss');
  const coreScss = join(config.coreRoot, 'scss', 'main.scss');
  const srcScss = existsSync(consumerScss) ? consumerScss : coreScss;
  const destCss = join(config.outputDir, 'css', 'main.css');

  ensureDir(join(config.outputDir, 'css'));

  try {
    execSync(`npx sass "${srcScss}":"${destCss}" --style=compressed`, {
      cwd: config.projectRoot,
      stdio: 'pipe'
    });
    const label = existsSync(consumerScss) ? 'project' : 'core';
    console.log(`  ✅ css/main.css (${label})`);
  } catch (err) {
    console.error('  ⚠ SCSS compilation failed:', err.stderr?.toString().trim() || err.message);
  }
}
