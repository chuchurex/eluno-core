 ...
 * With baseLangPrefix: false, the base language has no prefix (root).
 */
export function langPrefix(config, lang) {
...
export function chapterUrl(config, lang, chapter) {
  ...
  return `${prefix}/chapters/${slug}.html`;
}
...
export function generateNavSidebar(config, chapter, allChapters, lang, ui, chapterSlugMap) {
  ...
  return `        <nav class="nav" id="sidebar">
            ...
            <div class="nav-back">
                <a href="${lp}/" class="nav-link">← ${ui.home}</a>
            </div>
            <div class="nav-section">
${chapterLinks}            </div>
            <div class="nav-back nav-back--footer">
                <a href="${lp}/glossary.html" class="nav-link">${ui.glossaryPage}</a>
                <a href="${lp}/about.html" class="nav-link">${ui.about}</a>
            </div>
        </nav>`;
}
...
export function generatePageNavSidebar(config, allChapters, lang, ui, currentPage) {
  const langSelector = config.languages
    .map((l, i) => {
      ...
      return `${prefix}<a href="${lp}/${currentPage}"${active} onclick="localStorage.setItem('lang','${l}')" aria-label="${ariaLabel}">${l.toUpperCase()}</a>`;
    })
    .join('');
...
${chapterLinks}            </div>
            <div class="nav-back nav-back--footer">
                <a href="${lp}/glossary.html" class="nav-link${glossaryActive}">${ui.glossaryPage}</a>
                <a href="${lp}/about.html" class="nav-link${aboutActive}">${ui.about}</a>
            </div>
        </nav>`;
}
...
export function generateChapterPrevNext(config, chapter, allChapters, lang, ui) {
  ...
  if (prevChapter) {
    ...
    const lp = langPrefix(config, lang);
    html += `                <a href="${lp}/" class="chapter-nav-link prev">\n`;
